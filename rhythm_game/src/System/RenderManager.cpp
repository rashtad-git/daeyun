...
std::stringstream RenderManager::OnRender(IRenderer *renderer) const
{
	...
	for (int y = 0; y < Config::SCREEN_HEIGHT; y++)
	{
		...
		{
			if (y == 0 || y == Config::SCREEN_HEIGHT - 1 || x == 0 || x == Config::SCREEN_WIDTH - 1)
			{
				// guide line
				buffer << "#";
			}
		...
		{
			char fpsBuffer[32];
			sprintf_s(fpsBuffer, sizeof(fpsBuffer), "   %.2f FPS", data.system.CurrentFPS);
			buffer << fpsBuffer;
		}
		...
		if (y == 6)
		{
			buffer << "   " << "Meter " << data.game.TimeSig.Top << "/" << data.game.TimeSig.Bottom;
		}
