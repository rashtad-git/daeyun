...
bool GameManager::onGameLoop()
{
    ...
    auto now = clock();
    double deltaTime = static_cast<double>(now - This->lastUpdateTime) / CLOCKS_PER_SEC;
    ...
    double renderInterval = static_cast<double>(now - This->lastRenderTime) / CLOCKS_PER_SEC;
    if (renderInterval >= 1.0 / DataManager::GetInstance().system.TargetFPS)
    {
        DataManager::GetInstance().system.CurrentFPS = 1.0 / renderInterval; // 초당 프레임 수
