...
void NodeController::OnUpdate(double deltaTime)
{
	...
	// 노드가 hit 되었을 때 이펙트와 pool로 되돌리는 처리
	OnCleanup(deltaTime);
...
void NodeController::OnSpawn(double deltaTime)
{
	...
	{
		// Beginner는 첫박만 1개의 노트 생성
	case Difficulty::Beginner:
		if (sigIndex == 0)
			break;
		return;

		// easy는 첫박에 최대 2개의 노트가 생성되고 박마다 가끔 노트가 생성
	case Difficulty::Easy:
		...
		return;
		// easy 와 같으나 더 많은 빈도로 노트 생성, 박에도 2개의 노트 등장
